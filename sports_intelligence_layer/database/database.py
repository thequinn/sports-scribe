...
class SoccerDatabaseQueryBuilder:
    ...
    async def _build_player_stats_query(self, parsed_query) -> QueryResult:
        ...
        # Find player entity
        player_entity = next(
            (e for e in parsed_query.entities if e.entity_type.value == "player"), None
        )
        ...
        if "opponent_tier" in parsed_query.filters:
            opponent_condition = self._get_opponent_tier_condition(
                parsed_query.filters["opponent_tier"]
            )
    ...
    async def _build_team_stats_query(self, parsed_query) -> QueryResult:
        ...
        team_entity = next(
            (e for e in parsed_query.entities if e.entity_type.value == "team"), None
        )
    ...
    async def _build_head_to_head_query(self, parsed_query) -> QueryResult:
        ...
        team_entities = [
            e for e in parsed_query.entities if e.entity_type.value == "team"
        ]
